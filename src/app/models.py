...
class Prompt(Base):
    ...
    ratings = relationship(
        "Rating", back_populates="prompt", cascade="all, delete-orphan"
    )
    ...
    def __repr__(self):
        return (
            f"<Prompt(id={self.id}, title='{self.title}', category='{self.category}')>"
        )
...
class PromptExecution(Base):
    ...
    token_usage = Column(
        JSON, nullable=True
    )  # 存储 token 使用情况，例如 {"prompt_tokens": 10, "completion_tokens": 20}
