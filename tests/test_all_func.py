...
def test_01_auth_system():
    ...
    if res.status_code == 400:
        pytest.fail(
            f"User {u1} already exists? Did you run tests"
            f" twice with same ID? DB might be very dirty."
        )
...
def test_03_versioning_and_cache_invalidation():
    ...
    with httpx.Client() as client:
        ...
        res_roll = client.post(
            f"{BASE_URL}/prompts/{prompt_id}/rollback/1", headers=headers
        )
...
def test_04_tags():
    ...
    with httpx.Client() as client:
        ...
        res_tag = client.post(
            f"{BASE_URL}/tags", json={"name": tag_name}, headers=headers
        )
...
def test_06_llm_execution():
    ...
    with httpx.Client() as client:
        ...
        exec_res = client.post(
            f"{BASE_URL}/prompts/{pid}/execute",
